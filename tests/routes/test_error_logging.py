...
def _insert_account(conn: sqlite3.Connection, *, account_id: int) -> None:
    ...
    conn.execute(
        ...
        VALUES (?, 'tester@example.com', 'imap.example.com', 143, 'tester@example.com', ?, 0,
                'smtp.example.com', 587, 'tester@example.com', ?, 0, 1)
        """,
        (
            account_id,
            encrypted_password,
...
def test_fetch_emails_logs_starttls_failure(monkeypatch, client, test_db_path, caplog):
    with sqlite3.connect(test_db_path) as conn:
        _insert_account(conn, account_id=1)
        conn.commit()
    ...
    response = client.post('/api/fetch-emails', json={'account_id': 1})
