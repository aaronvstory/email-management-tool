...
# will attempt a lazy import inside the detector and completely skip on Windows.
def _magic_detect(file_bytes: bytes) -> Optional[str]:
    """Best-effort MIME detection using python-magic when safe.

    Returns a MIME string or None if unavailable. Intentionally avoids any
    import on Windows to prevent hard crashes due to ctypes DLL load issues.
    """
    try:
        import os as _os
        if _os.name == 'nt':
        ...
        try:
            _magic = _importlib.import_module('magic')  # python-magic
        except Exception:
            return None
        try:
            return _magic.from_buffer(file_bytes, mime=True)  # type: ignore[attr-defined]
        except Exception:
            return None
    except Exception:
        # Absolute safety: never let MIME detection crash the process
        return None
...
def _detect_mime_type(file_bytes: bytes, filename: Optional[str] = None) -> str:
    detected: Optional[str] = _magic_detect(file_bytes)
